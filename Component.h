...
public:
    ...
    { throw Error(get_name() + ": I cannot add!"); }
    virtual void remove_component(const std::string& name);

    // todo ?? virtual is unecessary todo use assert?
    virtual bool is_moving() const { throw Error(get_name() + " is not an object!"); }
    virtual bool is_alive() const { throw Error(get_name() + " is not an object!"); }
